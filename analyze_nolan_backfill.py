#!/usr/bin/env python3
"""Compose frozen learned CPRC with fixed and support-gated NoLan fallbacks."""
...
def run_model(
    ...
    cprc_result = cprc_artifact["experiments"][model][
        "selected_by_development_cs_budget"
    ][primary_family][budget]
    cprc_predictions = reconstruct_primary(
        rows,
        geometry,
        cprc_result["params"],
    ...
    raw_union = backfill_predictions(rows, cprc_predictions, raw_nolan)
    ...
    for gate in family_gate_grid("map_no_laplace_or_density", (float("inf"),)):
        ...
        selector.consider(
            "nolan_support_backfill",
            {
                ...
                "arbitration": "retain CPRC override; otherwise use gated NoLan proposal",
                "gate": gate,
            },
            backfill_predictions(rows, cprc_predictions, gated_nolan),
            nolan_details,
        )
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/cprc_nolan_backfill_v1.json")
    parser.add_argument(
        "--cprc-artifact",
    ...
    cprc_path = Path(args.cprc_artifact)
    config = json.loads(config_path.read_text(encoding="utf-8"))
    cprc = json.loads(cprc_path.read_text(encoding="utf-8"))
    payload = {
        "version": VERSION,
        "config": str(config_path),
        "config_sha256": sha256(config_path),
        "cprc_artifact": str(cprc_path),
        "cprc_artifact_sha256": sha256(cprc_path),
        "selection_split": "development_only",
        "test_refit": False,
        "models": {},
    }
    for model, spec in config["models"].items():
        payload["models"][model] = run_model(model, spec, config, cprc)
    ...
    for model, result in payload["models"].items():
        for method in ("frozen_cprc", "nolan_raw_backfill", "nolan_support_backfill"):
            for task in ("mc", "qa"):

...
def _load_model_specs(models_arg: str) -> List[ModelSpec]:
    ...
    for entry in raw:
        ...
        models_root = str(entry.get("models_root") or os.environ.get("CDH_MODELS_ROOT") or "models")
...
def _logit_lens_logp_from_hidden(model: Any, hidden: Any) -> Tuple[Any, Dict[str, Any]]:
    ...
    target_dtype = getattr(weight, "dtype", normed.dtype)
    target_device = getattr(weight, "device", normed.device)
    with torch.inference_mode():
...
def main() -> int:
    ...
    ap.add_argument("--jsonl", default="CDH-Bench.revised.strict.jsonl")
    ...
    if not models_arg:
        ...
        specs = [
            ModelSpec(
                ...
                base_url=base_url, temperature=temperature, max_tokens=max_tokens, models_root="models"
            )
        ]
    else:
